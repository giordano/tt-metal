...
a channel depth, $C$, of $32$ (see Figure 1).
 Let us input two such images, setting batch
...
where the order of the dimensions is $[N, H, W, C]$.

<img src="media/input.svg" style="width:300px;">\
_Figure 1: Input_

Let us perform the
convolution with filters of size $[3,3]$ (see Figure 2),
using a stride of $2$ in both $H$ and $W$
...
<img src="media/filters.svg" style="width:600px;">\
_Figure 2: Filters_
...
and height of the input image. In a matrix multiply a dot product is
...
by the kernel window. This is illustrated in the Figure 3.
...
<img src="media/transformation.svg" style="width:600px;">\
_Figure 3: Input transformation_
...
<img src="media/output.svg" style="width:300px;">\
_Figure 4: Output_
...
Next, we visualize what our window looks like in the Figure 10: C.
Recall that the weight tensor consists of a number of filters,
and each filter consists of one or more kernels. This kernel is also
referred to as the *window* in 2D visualization representation.
We can see the 3D kernels (a single filter) that is being convoluted
across our input tensor.
...
**The goal of halo is to figure out which input data is needed for output
data elements calculated on a specific core, and to bring that data to the core.**

### Halo implementation


Halo op performs the needed data movemenet in the following manner:
1. allocate buffer of the size needed to fit the local input shard sticks,
padding sticks and sticks of the remote shards needed to obtain the output shard;
this buffer will be the output of the halo op
2. fill padding pixel sticks in the allocated buffer with padding value
...
\* If we pass **true** for `remote_read` argument of the halo_op, then each core
...
flag is triggered, remote data movement is done by the core that consumes the data (src-remote, dst-local).

To perform steps 2-4, halo kernel needs to know for each stick in the halo output
(which will be convolution input), whether it is padding, it comes from the local
shard or from a remote shard. This metadata is calculated by the sliding window
infrastructure that is called within the halo operation. The final config is pushed
to the device as a sharded tensor to the reserved part of memory - currently we reserve
this memory using `l1_smal_size config` when initializing the device.
From here, config tensors are used by the halo kernel to perform needed transfers.


Let's explain how this config is generated, step by step. We will focus on the 2D
representation, since we are using the Height shard example, and we are not splitting the channels.
The steps are all functions defined in sliding_window namespace called from
[the halo op](https://github.com/tenstorrent/tt-metal/blob/main/ttnn/cpp/ttnn/operations/sliding_window/halo/device/halo_device_operation.cpp), in the create_program function, before the kernels are dispatched.

#### Step 1 - Pad metadata

Let's now expand our example to include the following padding (see Figure 11):
- $pad_h$ = 1
- $pad_w$ = 1
- padding value = 0
...
- start & end stick indices of the input tensor needed for the output shard
...
input elements were needed to compute that (see Figure 14). We
...
<img src="media/inputpercore.png" style="height:450px;">\
_Figure 15: Halo output for each core (convolution input)_
...
Using the information from steps 1-4, we generate 3 config tensors: padding, local and remote configs.

##### Padding config

This is a vector of pairs, where first element represents the local index
in the halo output shard where padding should start, and the second one represents the length.
...
- number of elements describing chunks of local shard needed - 3*number of chunks (1 vector elements)
- for each consecutive chunk of sticks
    - index of the first stick of the chunk in the local input shard
    - index in the first stick of the chunk in the full padded input
    - chunk length

<img src="media/localconfig.png" style="height:200px;">\
_Figure 17: Local input shard chunks for core 0_

For core 0, this vector would describe 2 chunks needed by core 1 (see Figure 17):
| Element | Note   |
|-------|-----------|
| 18     | _core x_ |
| 18     | _core y_ |
...
| 9      | _chunk 1 first stick index in the full padded input (see Figure 12)_
| 6      | _chunk 1 length_
| 6      | _chunk 2 first stick index in the local input shard_
| 17     | _chunk 2 first stick index in the full padded input (see Figure 12)_
| 2      | _chunk 2 length_


##### Remote config

Remote config has same structure as local config, only now for each chunk we also need core x,y info, since we are sending data to a remote core.

<img src="media/remoteconfig.png" style="height:200px;">\
_Figure 18: Remote input shard chunks for core 1_
...
| 1      | _chunk 1 first stick index in the local input shard_
| 10     | _chunk 1 first stick index in the full padded input (see Figure 12)_
| 5      | _chunk 1 length_
| 7      | _chunk 2 first stick index in the local input shard_
| 8      | _chunk 2 first stick index in the full padded input (see Figure 12)_
| 2      | _chunk 2 length_
