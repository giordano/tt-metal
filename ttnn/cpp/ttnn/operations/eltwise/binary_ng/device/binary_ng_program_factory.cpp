...
std::tuple<uint32_t, uint32_t> calculate_compute_kernel_args(
    ...
    uint32_t start_t = start_tile_id % Ht;
...
std::tuple<uint32_t, uint32_t> get_shards_per_height_width(
    ...
    if (memory_layout == tt::tt_metal::TensorMemoryLayout::HEIGHT_SHARDED) {
        return {num_cores, 1};
    } else if (memory_layout == tt::tt_metal::TensorMemoryLayout::WIDTH_SHARDED) {
        return {1, num_cores};
    } else {
        auto bbox = all_cores.bounding_box();
        if (orientation == ShardOrientation::ROW_MAJOR) {
            return {bbox.end_coord.y - bbox.start_coord.y + 1, bbox.end_coord.x - bbox.start_coord.x + 1};
        } else {
            return {bbox.end_coord.x - bbox.start_coord.x + 1, bbox.end_coord.y - bbox.start_coord.y + 1};
        }
    }
}
...
void set_or_update_runtime_arguments(
    ...
    const bool has_partial_rows = shard_spec.has_value() && (memory_layout != TensorMemoryLayout::HEIGHT_SHARDED);
    ...
    if (shard_spec.has_value()) {
        ...
        if (zero_start_grid) {
            ...
            cores = grid_to_cores_with_noop(grid, all_device_cores, row_major);
        }
    } else {
        if (zero_start_grid) {
            std::tie(
    ...
    auto [num_shards_per_height, num_shards_per_width] =
        shard_spec.has_value() ? get_shards_per_height_width(all_cores, memory_layout, shard_spec->orientation)
                               : std::tuple{0u, 0u};
