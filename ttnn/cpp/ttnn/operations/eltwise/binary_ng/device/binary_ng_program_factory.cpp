...
BinaryNgDeviceOperation::ProgramFactory::cached_program_t BinaryNgDeviceOperation::ProgramFactory::create(
    ...
    auto compute_kernel_defines = is_sfpu_op ? op_config.as_sfpu_defines(a.get_dtype()) : op_config.as_defines();
    ...
    if (is_sfpu_op && a.get_dtype() == DataType::FLOAT32) {
        reader_defines["SFPU_F32"] = "1";
    } else if (is_sfpu_op && a.get_dtype() == DataType::INT32) {
        reader_defines["SFPU_INT32"] = "1";
    } else {
        reader_defines["FPU_BF16"] = "1";
    }
    writer_defines = reader_defines;
