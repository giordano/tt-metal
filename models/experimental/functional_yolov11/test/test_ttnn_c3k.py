...
def test_yolo_v11_bottleneck(
    ...
    print("shape  and lyaut", ttnn_input.shape, ttnn_input.layout)
    torch_output = torch_module(torch_input)
    parameters = create_yolov11_model_parameters(torch_module, torch_input, device=device)
    ttnn_module = ttnn_c3k(device=device, parameter=parameters, conv_pt=torch_module)
    ttnn_output = ttnn_module(x=ttnn_input, device=device)  # ttnn.Shape([1, 1, 224, 64])
    print("output shapes", torch_output.shape, ttnn_output.shape)
